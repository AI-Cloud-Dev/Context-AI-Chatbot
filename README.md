...
![Redis](https://img.shields.io/badge/Redis-Memory%20%26%20Rate%20Limiting-dc382d?logo=redis)
...
| Feature | Details |
|---|---|
| 🔐 JWT Authentication | Secure register/login, token-protected endpoints, user isolation |
| 📄 Document Upload | Supports PDF, DOCX, XLSX with text extraction pipeline |
| 🧠 RAG Pipeline | Chunk → Embed → Store → Retrieve → Generate |
| 💬 Chat Memory | Per-user conversation history stored in Redis |
| ⚡ Rate Limiting | Redis-based per-user request throttling |
| 🔍 Semantic Search | Embedding-based document retrieval via ChromaDB |
| 🤖 Local LLM | Ollama inference — no external API costs |
| 🖥️ Full-Stack UI | ChatGPT-style Streamlit interface |
...
- [Ollama](https://ollama.ai) installed and running (`ollama run llama3`)
- (Optional) Docker

### 1. Clone the repository

```bash
git clone https://github.com/AI-Cloud-Dev/Context-AI-Chatbot.git
...
MIT License — feel free to use this as a reference or starting point for your own RAG systems.
